...
if [ -z "$SYSLOGIP" ];
then
    sudo docker run --name $CONTAINER_NAME --privileged -i -t -d -p 80:80 -p 5665:5665 -p 8080:8080 -p 39001:39001 -v /tmp/commands.conf:/etc/icinga2/conf.d/commands.conf -v /tmp/services.conf:/etc/icinga2/conf.d/services.conf -v /tmp/groups.conf:/etc/icinga2/conf.d/groups.conf $IMAGE_NAME /sbin/init
else
    sudo docker run --name $CONTAINER_NAME --privileged -i -t -d -p 80:80 -p 5665:5665 -p 8080:8080 -p 39001:39001 -v /tmp/commands.conf:/etc/icinga2/conf.d/commands.conf -v /tmp/services.conf:/etc/icinga2/conf.d/services.conf -v /tmp/groups.conf:/etc/icinga2/conf.d/groups.conf -e "SYSLOGIP=$SYSLOGIP" $IMAGE_NAME /sbin/init
fi
